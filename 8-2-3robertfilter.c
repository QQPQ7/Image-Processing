...
int main()
{
		...
		for (int i = 0; i < width; i++)
		{
			if ( j == height - 1 || i == width - 1)
			{
				if (j == height - 1 && i == width - 1)
					...
					mean = (-Y1[(j)*width + i] + Y1[(j + 1) * width + i]);
				else if (j == 0)
			...
			{
				mean = (-Y1[(j ) * width + i] + Y1[(j + 1) * width + i + 1]);
			}
			if (mean > 255)
				mean = 255;
			else if (mean < 0)
	...
	for (int j = 0; j < height; j++)
	{
		for (int i = 0; i < width; i++)
		{
			...
			{
				if (j == 0 && i == 0)
					mean = (-Y1[(j)*width + i] - 2 * Y1[(j)*width + i + 1] - Y1[j * width + i] + Y1[(j + 1) * width + i + 1] + 2 * Y1[(j + 1) * width + i] + Y1[(j + 1) * width + i + 1]);
				else if (j == height - 1 && i == width - 1)
					mean = (-Y1[(j - 1) * width + i - 1] - 2 * Y1[(j - 1) * width + i] - Y1[j * width + i - 1] + Y1[j * width + i] + 2 * Y1[(j)*width + i - 1] + Y1[(j)*width + i]);
				else if (j == 0)
					mean = (-Y1[(j)*width + i - 1] - 2 * Y1[(j)*width + i + 1] - Y1[j * width + i - 1] + Y1[j * width + i + 1] + 2 * Y1[(j + 1) * width + i - 1] + Y1[(j + 1) * width + i + 1]);
				else if (i == 0)
					mean = (-Y1[(j - 1) * width + i] - 2 * Y1[(j - 1) * width + i + 1] - Y1[j * width + i] + Y1[j * width + i + 1] + 2 * Y1[(j + 1) * width + i] + Y1[(j + 1) * width + i + 1]);
				else if (j == height - 1)
					mean = (-Y1[(j - 1) * width + i - 1] - 2 * Y1[(j - 1) * width + i + 1] - Y1[j * width + i - 1] + Y1[j * width + i + 1] + 2 * Y1[(j)*width + i - 1] + Y1[(j)*width + i + 1]);
				else if (i == width - 1)
					mean = (-Y1[(j - 1) * width + i - 1] - 2 * Y1[(j - 1) * width + i] - Y1[j * width + i - 1] + Y1[j * width + i + 1] + 2 * Y1[(j + 1) * width + i - 1] + Y1[(j + 1) * width + i]);
			}
			else
			{
	...
	FILE* outputFile = fopen("OutputRobertslFilter.bmp", "wb"); //출력체크용
