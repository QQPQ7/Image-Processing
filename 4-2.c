...
int main()
{
	...
	inputFile2 = fopen("AICenterY_Noise.bmp", "rb");//rb 바이너리로 읽기
	...
	for (int j = 0; j < height; j++)
	{
		for (int i = 0; i < width; i++)
		{
			...
			//Y1[j * width + i] += rand() % Err - (Err >> 1);
			if ((rand() % Err) == 0) Y1[j * width + i] = 255;
			else if ((rand() % Err) == 1) Y1[j * width + i] = 0;
			else Y1[j * width + i] = Y1[j * width + i];
	...
	FILE* outputFile = fopen("OutputNoise.bmp", "wb"); //출력체크용
