...
int main()
{
	...
	double a, b, Grad;
	Y1 = (double*)calloc(width * height, sizeof(double));
	...
	*/
	...
	for (int j = 0; j < height; j++)
	{
		for (int i = 0; i < width; i++)
		{
			...
			if ((int)Y2[j * width + i] < -3)
				fprintf(txtoutputFile, "%s ", bi[0]);
			else if ((int)Y2[j * width + i] == -3)
				fprintf(txtoutputFile, "%s ", bi[1]);
			else if ((int)Y2[j * width + i] == -2)
				fprintf(txtoutputFile, "%s ", bi[2]);
			else if ((int)Y2[j * width + i] == -1)
				fprintf(txtoutputFile, "%s ", bi[3]);
			else if ((int)Y2[j * width + i] == 0)
	...
	for (int j = 0; j < height; j++)
	{
		for (int i = 0; i < width; i++)
		{
			...
			Y3[j * width + i] = Y3[j * width + i] * div;
			Y2[j * width + i] = (int)Y2[j * width + i] * div;
			//printf("%d ", Y3[j * width + i]);
			if (i == 0)
			{
				Y4[j * width + i] = Y3[j * width + i] + 128;
				//printf("%d ", Y4[j * width + i]);
			}
			else
			{
				if (Y4[j * width + i - 1] + Y3[j * width + i] > 255)
				{
					Y4[j * width + i] = 255;
					//mean = 255;
					//printf("%d ", Y4[j * width + i]);
				}
				else if (Y4[j * width + i - 1] + Y3[j * width + i] < 0)
				{
					//mean = 0;
					//printf("%d ", mean);
					Y4[j * width + i] = 0;
				...
				else
				{
					...
					Y4[j * width + i]  = Y4[j * width + i-1 ] + Y3[j * width + i];
					//printf("%d ", Y4[j * width + i]);
				}
				//Y4[j * width + i] =mean;
